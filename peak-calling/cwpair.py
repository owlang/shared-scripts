...
import csv, gzip, bisect, sys, logging, os, traceback, math
...
def perform_process(path, match_func, threshold, options):
    ...
    if is_gz_file(path):
        input = csv.reader(gzip.open(path,'rt'), delimiter='\t')
    else:
    ...
    def gz_writer(output_type, extension='txt.gz'): # Returns a tab-delimited gzip-writer for a certain output
        return csv.writer(gzip.open(make_path(output_type, extension), 'wt'), delimiter='\t')
    ...
    if options.gzip:
        summary_output = gz_writer('S', extension=options.format)
        detailed_output = gz_writer('D')
        orphan_output = gz_writer('O')
    else:
    ...
    for cname, chromosome in chromosomes.items():
        ...
        for peak in watson:
            ...
            if match:
                ...
                dist.add(d)
                if 'txt' in options.format:
                    x.append((cname, midpoint, midpoint+1, peak[3]+match[3], d))
                else: #gff
                    x.append(gff_row(cname, source='cwpair', start=midpoint, end=midpoint+1,
    ...
    for row in x:
        summary_output.writerow(list(row))
...
if __name__ == '__main__':
    ...
    parser.add_option('-F', action='store', type='float', dest='absolute_threshold', default=0,
                      help='Absolute read filter; outputs only peaks with larger read count. Default 0 (no filtering).')
    ...
    if options.gzip:
        options.format = options.format + ".gz"
