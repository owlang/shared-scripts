...
if __name__ == '__main__':
    ...
    for data in payload:
        ...
        biological_replicate = data.get('biological_replicates', ["-1"])[0]
        ...
        technical_replicates = data.get('technical_replicates', [None])[0]
        ...
        FQ1 = FQ2 = platform = perturbed = ref_file = genome = None
        for dfile in data.get("derived_from",[]):
            durl = "https://www.encodeproject.org" + dfile + "?format=json"
            ddata = fetch_data(durl)
            output_category = ddata.get('output_category', None)
            if (output_category == "reference"):
                ref_file = dfile
                genome = ddata.get('output_type', None)
        ...
        row = [accession, md5sum, assembly, mapped_run_type, ref_file, genome, FQ1, FQ2, mapped_read_length, output_category, output_type, platform, ENCSR, biological_replicate, strain, assay_title, target, technical_replicates, date_created, file_size]

        lines.append('\t'.join([str(i) for i in row]))

    headers = ["accession", "md5sum", "assembly", "mapped_run_type", "ref_file", "genome", "FQ1","FQ2", "mapped_read_length", "output_category", "output_type", "platform", "ENCSR", "biological_replicate", "strain", "assay_title", "target", "technical_replicates", "date_created", "file_size"]
