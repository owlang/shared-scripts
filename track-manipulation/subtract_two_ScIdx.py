...
This script subtracts signal between two ScIdx files to output a new ScIdx file.

Example: python subtract_two_ScIdx.py -a SIGNAL.scidx -b BACKGROUND.scidx > RESULTS.scidx''')
	parser.add_argument('-a','--signal', metavar='scidxfile', dest='signal_fn', required=True, help='a ScIdx file to subtract from')
	parser.add_argument('-b','--background', metavar='scidxfile', dest='background_fn', required=True, help='a ScIdx file to subtract by')
	parser.add_argument('-o','--output', metavar='outfile', dest='output_fn', required=True, help='a ScIdx file of subtracted signal')
...
if __name__ == "__main__":
	...
	b_info = loadSCIDX(args.background_fn,'b')
	merge_info = loadSCIDX(args.signal_fn,'s')
